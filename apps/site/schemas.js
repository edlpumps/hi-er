...
exports.init = function init(mongoose) {
  ...
  var PasswordResets = mongoose.model(
    "password_resets",
    resetSchema,
    "password_resets",
  );
  ...
  var subscribers = mongoose.model(
    "subscribers",
    subscriber_schema,
    "subscribers",
  );
  ...
  var certificate_transactions = mongoose.model(
  ...
  var certificates = mongoose.model(
    "certificates",
    certificateSchema,
    "certificates",
  );
  ...
  pumpSchema.statics.search = async (participant, search, skip, limit) => {
    ...
    const pumps = await Pumps.find(query)
      .sort({
        basic_model: 1,
        individual_model: 1,
      })
  ...
  pumpSchema.statics.countsByParticipant = async (listed, participants) => {
    ...
    if (participants) {
      ...
      for (const count of result) {
        const i = participants
          .map((p) => p._id.toString())
          .indexOf(count._id.toString());
        if (i >= 0) {
  ...
  var participants = mongoose.model(
    "participants",
    participant_schema,
    "participants",
  );
