...
const svg_opts = {
  font: {
    fontFiles: [
      path.join(__dirname, "../utils/fonts/Arimo-Regular.ttf"),
...
const hi_logo_data_uri_en = fs.readFileSync(
  path.join(
    __dirname,
    "../views/svg/" + i18next.t("hi_label_logo", {lng: "en"}),
  ),
  "utf-8",
);
const hi_logo_data_uri_small_en = fs.readFileSync(
  path.join(
    __dirname,
    "../views/svg/" + i18next.t("hi_label_logo_small", {lng: "en"}),
  ),
  "utf-8",
);
const hi_logo_data_uri_fr = fs.readFileSync(
  path.join(
    __dirname,
    "../views/svg/" + i18next.t("hi_label_logo", {lng: "fr"}),
  ),
  "utf-8",
);
const hi_logo_data_uri_small_fr = fs.readFileSync(
  path.join(
    __dirname,
    "../views/svg/" + i18next.t("hi_label_logo_small", {lng: "fr"}),
  ),
  "utf-8",
);
const hi_approval_check_uri = fs.readFileSync(
  path.join(__dirname, "../views/svg/hi-approval-check"),
  "utf-8",
);
...
const label_sm_template_file = path.join(
  __dirname,
  "../views/svg/label-sm.pug",
);
const label_template = pug.compileFile(label_template_file);
const label_sm_template = pug.compileFile(label_sm_template_file);

const circulator_label_template_file = path.join(
  __dirname,
  "../views/svg/circulator-label.pug",
);
const circulator_small_label_template_file = path.join(
  __dirname,
  "../views/svg/circulator-label-small.pug",
);
...
var build_label_params = function (pump, label, req, res) {
  var label_lang = lang.get_label_language(req, res);
  var pm =
    pump.configuration == "bare"
      ? "- " + i18next.t("bare_pump", {lng: label_lang})
      : "- " + i18next.t("motor", {lng: label_lang});
  var config =
  ...
  var load =
    pump.configuration == "bare" || pump.configuration == "pump_motor"
      ? i18next.t("constant_load", {lng: label_lang})
      : i18next.t("variable_load", {lng: label_lang});
  ...
  var load_abbr =
    pump.configuration == "bare" || pump.configuration == "pump_motor"
      ...
      : "VL";
  var datetime =
    label.date.getTime() < pump.date.getTime() ? pump.date : label.date;
  ...
  var annual_cost_savings = common.calculate_cost_savings(
    pump.energy_rating,
    pump.motor_power_rated,
  );
  var annual_energy_savings = common.calculate_energy_savings(
    pump.energy_rating,
    pump.motor_power_rated,
  );
  var loc_dict = {
...
exports.make_label = function (req, participant, pump, label, res) {
  var params = build_label_params(pump, label, req, res);
  var output = label_template(params);
  return output;
};

exports.make_sm_label = function (req, participant, pump, label, res) {
  var params = build_label_params(pump, label, req, res);
...
var build_circulator_params = function (pump, waip, max, req, res) {
  ...
  try {
    annual_cost_savings = common.calculate_circulator_cost_savings(
      pump.least.energy_rating.toFixed(),
      waip.toFixed(3),
    );
    console.log("Cost Savings " + JSON.stringify(annual_cost_savings, null, 2));
    annual_energy_savings = common.calculate_circulator_energy_savings(
      pump.least.energy_rating.toFixed(),
      waip.toFixed(3),
    );
    console.log(
      "Energy Savings " + JSON.stringify(annual_energy_savings, null, 2),
    );
  } catch (e) {
  ...
  let retval = {
    ...
    small_logo:
      label_lang == "en"
        ? hi_logo_data_uri_small_en
        : hi_logo_data_uri_small_fr,
...
const calc_circ_vals = function (pump) {
  ...
  const nominal_hp =
    pump.least.output_power[3] / (pump.least.water_to_wire_efficiency / 100);
...
exports.make_circulator_label = function (req, participant, pump, res) {
  const circulator_label_template = pug.compileFile(
    circulator_label_template_file,
  );
  const {waip, maxScale} = calc_circ_vals(pump);
  return circulator_label_template(
    build_circulator_params(pump, waip, maxScale, req, res),
  );
};
exports.make_circulator_label_small = function (req, participant, pump, res) {
  const circulator_label_template = pug.compileFile(
    circulator_small_label_template_file,
  );
  const {waip, maxScale} = calc_circ_vals(pump);
  return circulator_label_template(
    build_circulator_params(pump, waip, maxScale, req, res),
...
exports.make_circulator_qr = function (req, participant, pump, res) {
  const port = req.app.settings.port || cfg.port;
  ...
  datauri.format(".png", code);
  const {waip, maxScale} = calc_circ_vals(pump);
  var qr_params = build_circulator_params(pump, waip, maxScale, req, res);
  qr_params.qr = datauri.content;
  return qr_template(qr_params);
};
