...
var configure = function () {
  ...
  app.use(function (req, res, next) {
    ...
    req.getNextCertificateOrderNumber =
      req.app.locals.db.getNextCertificateOrderNumber;
  ...
  app.use(function (req, res, next) {
    ...
    res.locals.certificate_cart_exists = req.session.certificate_cart
      ? req.session.certificate_cart.length > 0
      : false;
    ...
    res.locals.is_beta = undefined;
    if (process.env.NODE_ENV) {
      if (!process.env.NODE_ENV.toUpperCase().includes("PROD")) {
        res.locals.is_beta = process.env.NODE_ENV.toUpperCase();
      }
    }
    res.locals.db_connection = data_connection_str
      .split("?")[0]
      .split("27017/")
      .at(-1);
...
var conn = mongoose.connect(data_connection_str, {}, function (err, res) {
  if (err) {
    mainlog.fatal(
      "Could not connect to mongo database at %s",
      data_connection_str,
    );
  } else {
    if (
      process.env.NODE_ENV &&
      process.env.NODE_ENV.toUpperCase().includes("DEV")
    ) {
      ...
      app.locals.db.Users.findOne(
        ...
        function (err, user) {
          ...
          if (user) {
            ...
            mainlog.info(
              "Authentication of " + email + " failed (user unknown)",
            );
            return done(
              null,
              false,
...
var startup = function () {
  mainlog.info(
    "HI Energy Rating application startup -- %s, port %s",
    process.env.NODE_ENV,
    port,
  );
...
const push_emails = async function (interval, override) {
  if (
    process.env.NODE_ENV &&
    !process.env.NODE_ENV.toUpperCase().includes("PROD")
  ) {
    console.log(
      "Skipping email push interval[" + interval + "] in development/beta mode",
    );
    return;
  }
  try {
    const {pumps, circulators, certificates} = await exporter.create("all", {
      admin: false,
    });
